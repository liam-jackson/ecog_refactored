...
fig_name1 = ['stim_baag_perm_auc_left'];
fig_filename1 = [p.times_label, '_', p.current_group_value, '_', fig_name1, '.fig'];
png_filename1 = [fig_name1, '.png'];

stim_baag_perm_fig = figure('FileName', png_filename1);
stim_baag_perm_fig.WindowState = 'maximized';

sgtitle({'Distribution of AUC, Z-scores of Classifiers Trained on', 'Randomized Individual Electrode Stimulus Data', ['Class Label: BAAG. ', p.current_group_value, ' electrodes']}, 'FontSize', title_size);
subplot(1,2,2);
zscore_hist = gca;
hold on;
...
savefig(fullfile(p.figs_quickref_path, fig_filename1));
savefig(fullfile(p.figs_quickref_path, fig_filename2));
